...
class SimpleZoneMonitor:
    ...
    def update(self, track_id: int, bottom_center: Tuple[float, float], bbox_wh: Tuple[float, float], now_ts: float, fps_hint: float=30.0):
        ...
        pre = self.in_prefall(bottom_center)
        st.last_prefall = pre
        event = None
        ...
        # return a small dict for UI/OSD coloring and optional event
        return {"prefall": pre, "dwell": st.dwell, "event": event}
