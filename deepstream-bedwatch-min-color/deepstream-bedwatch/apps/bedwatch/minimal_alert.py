...
#!/usr/bin/env python3
import sys, time, argparse, yaml, os
import gi
...
def set_box_color(obj_meta, r, g, b, a=1.0):
    # Set border color & width for NvOSD
    ...
    while l_frame is not None:
        ...
        while l_obj is not None:
            ...
            if obj_meta.class_id == u_data["person_class_id"]:
                ...
                res = zm.update(track_id, bottom_center, (width, height), now, fps_hint=fps_hint)
                # color: red if in prefall, green otherwise
                if res.get("prefall"):
                    set_box_color(obj_meta, 1.0, 0.0, 0.0, 1.0)
                else:
                    set_box_color(obj_meta, 0.0, 1.0, 0.0, 1.0)
                # persist status JSON for GitHub commit
                write_status(STATUS_PATH, cam_id, track_id, res.get("prefall"), res.get("dwell", 0.0))
                # optional console when fired
                if res.get("event") == "ALERT":
                    console_alert(cam_id, track_id, "ALERT", f"dwell={res.get('dwell',0):.1f}s")
...
def main():
    ...
    print("[INFO] minimal alert+color app starting...")
